...
def annotate_guides(cut_site, ann_db, feature):
    ...
    if feature is not None:
        ...
        if feature == 'intergenic' and 'gene' in feature_types:
            ...
            exon_name = ', '.join([exon for exon in set(pdFeatures.get('Name')) if bool(exon)]) # ', '.joining() is just in case there's an intron so small that a guide is somehow on two exons. Remove if unnecessary.
...
def main():
    ...
    if feature is not None and feature != 'intergenic' and feature != 'intron':
    ...
    if not args.output_folder:
        ...
        if ann_db is not False:
            ...
            iterable_cut_sites = [(cut_site, ann_db, feature) for cut_site in cut_sites]
