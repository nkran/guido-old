...
def fetch_convar_score(cut_site, conservation, var_callset):
    ...
    if conservation:
        cut_site['cons_score'] = np.mean(
            conservation['joined/{}/score/'.format(chrom)][3, start:end]
        )
...
def apply_conservation_variation_score(
    ...
    joined
        ...
        mean_short_sliding =
        np.array(pd.DataFrame(np.mean(identity_short, axis=0)).rolling(20, win_type ='hamming', center=True).mean())
        mean_long_sliding =
        np.array(pd.DataFrame(np.mean(identity_long, axis=0)).rolling(20, win_type ='hamming', center=True).mean())

        mean_short_sliding_big =
        np.array(pd.DataFrame(np.mean(identity_short, axis=0)).rolling(5000, win_type ='hamming', center=True).mean())
        mean_long_sliding_big =
        np.array(pd.DataFrame(np.mean(identity_long, axis=0)).rolling(5000, win_type ='hamming', center=True).mean())
    '''
