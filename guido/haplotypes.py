...
#         if allele == 0:
#             print(ref_region[i]),
#             # print(type(ref_region[i]))
#         elif allele == 1:
#             print(alt_region[i].replace('\n', ''))
...
samples_callset = samples_callset.reindex(np.repeat(samples_meta.index.values, 2)).reset_index(drop=True)
...
# print(samples_pops.groupby('population').count())
samples_per_pop = samples_pops.groupby('population').count()
samples_per_pop.columns = ['count']

for hap, pop_dict in hap_populations:
    print('-----')
    print(hap)
    print('-')
    for pop, n in pop_dict.items():
        print(pop, n, round(float(n) / (samples_per_pop.loc[pop]['count'] * 2), 3))
